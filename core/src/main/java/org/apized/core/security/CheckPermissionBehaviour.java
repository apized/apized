...
public class CheckPermissionBehaviour implements BehaviourHandler<Model> {
  ...
  public void process(Class<Model> type, When when, Action action, Execution<Model> execution) {
    ...
    if (!allowed && model != null && model._getModelMetadata().getTouched().size() > 0) {
      ...
      for (String field : model._getModelMetadata().getTouched()) {
        ...
        if (ApizedConfig.getEngine().equals("micronaut")) {
          Optional<AnnotationValue<TypeDef>> dataType = Optional.ofNullable(property.getAnnotation(TypeDef.class));
          isJsonb = dataType.isPresent() && dataType.get().enumValue("type", DataType.class).equals(DataType.JSON);
        } else {
          Optional<AnnotationValue<Type>> dataType = Optional.ofNullable(property.getAnnotation(Type.class));
          isJsonb = dataType.isPresent() && dataType.get().classValue("value").equals(JsonType.class);
        }
